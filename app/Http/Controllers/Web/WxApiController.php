...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return boolean
     */
    protected function setCache($authname,$value,$expired){
        //TODO: set cache implementation
        S($authname, $value, $expired);
        $res = S($authname);
        if(empty($res)){
            return false;
        }
     ...
     * @return mixed
     */
    protected function getCache($authname){
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        $access_token = DB::table('access_token')->where('id', 1)->first();
        ...
        if ($result)
        {
            ...
            DB::table('access_token')->where('id', 1)->update(['access_token' => $this->access_token, 'time' => time()]);
            return $this->access_token;
        }
        return false;
    }
...
//            $this->setCache($authname,$this->api_ticket,$expire);
     ...
     * @return string
     */
    public function getOauthRedirect($callback,$state='sima',$scope='snsapi_userinfo'){
    ...
    /**
     * @desc 通过基础access_token和openid获取用户信息
     * @auth : simayubo
     * @param [type] $access_token
     * @param [type] $openid
     * @return [type] [description]
     */
    public function getUiUserInfo($access_token,$openid){
        $result = $this->http_get('https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$access_token.'&openid='.$openid.'&lang=zh_CN');
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
